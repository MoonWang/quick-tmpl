...
/**
 * 异步串行创建模板
 * @param {String} toRoot 目标路径(相对执行位置)
 * @param {String} tmpl 模板文件夹名称(template 目录下)
 * @param {Object} options 需要替换的配置
 * @param {Object} root 脚本执行根路径
 */
class CopyTmpl {
    constructor(toRoot, tmpl,  options, root) {
        this.tmpl = tmpl;
    ...
    delate() {
        // 先递归删除 root 下所有文件，然后把 root 文件也删了
        return rmdir(path.resolve(this.root, this.toRoot))
            .then(_ => {
                console.log(`${this.tmpl} 目录删除完成`);
            }).catch(err => {
                console.log(`${this.tmpl} 目录删除失败：`);
